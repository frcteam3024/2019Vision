...
def main_vision_loop():
    ...
    while ((len(targets) < 2) and limitLoopCount < 1):
        count = count + 1
...
def calc_strafe(vision_data):
    ...
    offset = 11.5 * (width / t_width)
    ratio = (center + offset) / pixels_x
    angle = (x_angle * ratio) - (.5 * x_angle)
    return angle * -1


def estimate_z_direction(vision_data):
    t1 = vision_data[0]
    t2 = vision_data[1]
    x1 = t1['center']
    x2 = t2['center']
    h1 = t1['height']
    h2 = t1['height']
    # determine height function as h(x)=mx+b, where x is the pixel x-coordinate:
    m = (h2 - h1) / (x2 - x1)
    b = h1 - m * x1
    outer_h1 = b
    outer_h2 = m * pixels_x + b
    outer_d1 = calc_distance(outer_h1)
    outer_d2 = calc_distance(outer_h2)
    return np.sign(outer_d2 - outer_d1)
    ...
    if len(vision_array) == 2:
        ...
        if check_1 or check_2:
            ...
            for height in heights:
                dist = calc_distance(height)
                distances.append(dist)
            d_mid = sum(distances)

        strafe = calc_strafe(vision_array)
        print('Distance: ', d_mid)
        print('Strafe: ', strafe)
        table.putNumberArray('VisionData', [d_mid, strafe])
