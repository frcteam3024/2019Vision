...
def main_vision_loop():
    ...
    while len(targets) < 2:
        ...
        for target in [target_left, target_right, target_OOR]:
            if target:
                if target['height']/target['width'] < 3:
        ...
        if count > 20:
            table.putBoolean('SeeTargets', False)
...
def calc_distance(heights):
    ...
    for h in heights:
        ...
        distances.append(d)
    dist_untuned = sum(distances)/len(distances)
    return dist_untuned*(46./77.) - 19  # linear compensation for angle and y-offset
...
def calc_strafe(vision_data):
    ...
    offset = 2.5 * (width/t_width)  # compensates for x-axis offset
    ratio = (center + offset)/pixels_x
    angle = (x_angle*ratio) - (.5*x_angle)
    angle = angle*-1
    if angle > 0:  # the 15/11 compensates for y-axis offset
        angle = angle*(15/12)
    if angle < 0:
        angle = angle*(15/7)
    return angle
...
while True:
    ...
    see_targets = distance > 0
    table.putBoolean('SeeTargets', see_targets)
